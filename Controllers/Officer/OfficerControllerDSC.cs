...
namespace ReactMvcApp.Controllers.Officer
{
    public partial class OfficerController : Controller
    {
                    ...
                    ICollection<ICrlClient>? crlList, IOcspClient? ocspClient, ITSAClient? tsaClient, int estimatedSize)
        {
            ...
            string subjectDN = chain[0].SubjectDN.ToString();

            // Find the CN part within the subject
            string signedBy = subjectDN.Split(',')
                                    .FirstOrDefault(part => part.Trim().StartsWith("CN="))?
                                    .Split('=')[1]
                                    .Trim() ?? "Unknown Signer";

            string appearanceText = $"Digitally signed by: {signedBy}\n" +
                                    $"Reason: {reason}\n" +
                                    $"Location: {location}\n" +
            ...
            SignatureFieldAppearance appearance = new("SignatureField");
