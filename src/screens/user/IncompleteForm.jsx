...
const IncompleteForm = () => {
  ...
  useEffect(() => {
    if (formSections.length > 0 && formDetails) {
      // Iterate over each section
      formSections.forEach((section, sectionIndex) => {
        // Iterate over each field in the section
        section.fields.forEach((field) => {
          // Check if the field is a district field by name (case insensitive)
          if (
            field.name.toLowerCase().includes("district") &&
            formDetails[field.name]
          ) {
            // Trigger the district change logic manually
            handleDistrictChange(sectionIndex, field, formDetails[field.name]);
          }
          if (
            field.name.toLowerCase().includes("applicantimage") &&
            formDetails[field.name]
          ) {
            setApplicantImagePreview(formDetails[field.name]);
          }
        });
      });
    }
  }, [formSections, formDetails]);
